...
if ($get_products_count > 0) {
    for ($i = 0; $i < $get_products_count; $i++) {
        ...
        for ($j = 0; $j < count($get_all_products_data); $j++) {
            ...
            $has_map_policy = $results[0]->has_map_policy;

            if(isset($has_map_policy) && ($has_map_policy == 1)){
                $price = $results[0]->list_price;
            }else{
                $price_new = $results[0]->standard_dealer_price + 8.5;
                $price = $price_new * 1.18;
